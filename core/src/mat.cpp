...
class Mat::Impl
{
  public:
    ...
    auto Upload(const void *hostData, std::size_t hostStride) const noexcept -> Status
    {
        if (deviceData_ == nullptr)
        {
    ...
    auto Download(void *hostData, std::size_t hostStride) const noexcept -> Status
    {
        if (deviceData_ == nullptr)
        {
            LogError("Device data is not allocated.");
    ...
    auto Wait() const noexcept -> Status
    {
#ifdef BUILD_WITH_TENSORRT
        if (stream_ != nullptr)
        {
    ...
    auto Channels() const noexcept -> std::size_t
    {
        return channels_;
    }

    auto BytesPerElement() const noexcept -> std::size_t
    {
    ...
    auto Stride() const noexcept -> std::size_t
    {
        return deviceStride_;
    }

    auto Shape() const noexcept -> std::array<int64_t, 4>
    {
...
{
    return impl()->Upload(hostData, hostStride);
}

auto Mat::Download(void *hostData, std::size_t hostStride) const noexcept -> Status
{
    return impl()->Download(hostData, hostStride);
}
